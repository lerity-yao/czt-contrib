...
const (
	...
	importValueRegex = `(?m)"\/?(?:[^/]+\/)*[^/]+\.(api|cron|rabbitmq)"`
	tagRegex         = `(?m)` + "`" + `[a-z]+:".+"` + "`"
)
...
func match(p *CztctlParserParser, text string) {
...
func checkVersion(p *CztctlParserParser) {
	v := getCurrentTokenText(p)
...
func checkImportValue(p *CztctlParserParser) {
	v := getCurrentTokenText(p)
...
func checkKeyValue(p *CztctlParserParser) {
	v := getCurrentTokenText(p)
...
func checkKeyword(p *CztctlParserParser) {
	v := getCurrentTokenText(p)
...
func checkKey(p *CztctlParserParser) {
	v := getCurrentTokenText(p)
	if IsGolangKeyWord(v) {
		notifyErrorListeners(p, fmt.Sprintf("expecting ID, found golang keyword: '%s'", v))
	}
...
func isNormal(p *CztctlParserParser) bool {
...
func isInterface(p *CztctlParserParser) {
	v := getCurrentTokenText(p)
	if IsGolangKeyWord(v) {
		notifyErrorListeners(p, fmt.Sprintf("expecting ID, found golang keyword: '%s'", v))
	}
...
func getCurrentTokenText(p *CztctlParserParser) string {
...
func setCurrentTokenText(p *CztctlParserParser, text string) {
	token := p.GetCurrentToken()
	if token == nil {
		return
	}
...
func notifyErrorListeners(p *CztctlParserParser, msg string) {
