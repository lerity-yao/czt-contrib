...
// VisitApi implements from cztctl.BaseCztctlParserVisitor
func (v *ApiVisitor) VisitApi(ctx *cztctl.ApiContext) any {
	var final Api
...
func (v *ApiVisitor) acceptService(root, final *Api) {
	for _, service := range root.Service {
		...
		for _, route := range service.ServiceApi.ServiceRoute {
			uniqueRoute := route.Route.RouteName.Text()
			if _, ok := final.routeM[uniqueRoute]; ok {
				v.panic(route.Route.RouteName, fmt.Sprintf("duplicate route '%s'", uniqueRoute))
			}
			final.routeM[uniqueRoute] = Holder

			handlerExpr := route.GetHandler()
			if handlerExpr == nil {
				v.panic(route.Route.RouteName, "mismatched handler")
			}
			if handlerExpr.Text() == "" {
...
// VisitSpec implements from cztctl.BaseCztctlParserVisitor
func (v *ApiVisitor) VisitSpec(ctx *cztctl.SpecContext) any {
	var root Api
...
func (a *Api) Format() error { return nil }
...
func (a *Api) Equal(v any) bool {
	...
	sort.Slice(expectingImport, func(i, j int) bool { return expectingImport[i].Value.Text() < expectingImport[j].Value.Text() })
	sort.Slice(actualImport, func(i, j int) bool { return actualImport[i].Value.Text() < actualImport[j].Value.Text() })
	...
	sort.Slice(expectingType, func(i, j int) bool { return expectingType[i].NameExpr().Text() < expectingType[j].NameExpr().Text() })
	sort.Slice(actualType, func(i, j int) bool { return actualType[i].NameExpr().Text() < actualType[j].NameExpr().Text() })
	...
	for index, each := range a.Service {
		ac := root.Service[index]
		if !each.Equal(ac) {
			return false
		}
	}
