...
type (
	...
	// ApiVisitor wraps cztctl.BaseCztctlParserVisitor to call methods which has prefix Visit to
	// visit node from the api syntax
	ApiVisitor struct {
		*cztctl.BaseCztctlParserVisitor
		debug    bool
		prefix   string
		infoFlag bool
	}
...
func (v *ApiVisitor) getDoc(t TokenStream) []Expr {
	return v.getHiddenTokensToLeft(t, cztctl.COMMENTS, false)
}

func (v *ApiVisitor) getComment(t TokenStream) Expr {
	list := v.getHiddenTokensToRight(t, cztctl.COMMENTS)
	if len(list) == 0 {
		return nil
	}
