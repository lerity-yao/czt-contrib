...
import (
	...
	apiutil "github.com/lerity-yao/czt-contrib/cztctl/api/apiutil"
...
// stringSet is a simple ordered set of strings, replacing collection.Set[string].
...
func genTypesWithGroup(dir string, cfg *config.Config, api *spec.ApiSpec) error {
	...
	typesBelongToFiles := make(map[string]*stringSet)
	...
	for _, v := range api.Service.Groups {
		...
		for _, v := range v.Routes {
			...
			if !ok {
				requestTypeFileSet = newStringSet()
			}
			...
			if !ok {
				responseTypeFileSet = newStringSet()
			}
	...
	typesInOneFile := make(map[string]*stringSet)
...
func writeType(writer io.Writer, tp spec.Type) error {
	...
	if err := writeMemberTypes(writer, structType.Members); err != nil {
		return err
	}
...
func writeMemberTypes(writer io.Writer, members []spec.Member) error {
