...
// RegisterGoctlHome register cztctl home path.
func RegisterGoctlHome(home string) {
	goctlHome = home
}

// JoinPackages joins Go package paths.
func JoinPackages(pkgs ...string) string {
	return strings.Join(pkgs, "/")
}

// MkdirIfNotExist creates a directory if not exists.
...
// GetGoctlHome returns the path value of the cztctl, the default path is ~/.cztctl, if the path has
...
// GetDefaultGoctlHome returns the path value of the cztctl home where Join $HOME with .cztctl.
...
// GetGitHome returns the git home of cztctl.
...
// GetAutoCompleteHome returns the auto_complete home of cztctl.
...
// GetCacheDir returns the cache dit of cztctl.
...
// Copy copies the src file to dest.
...
// Hash returns the md5 hash of the file.
...
// FindGoModPath finds go.mod and returns (module path, module dir).
func FindGoModPath(dir string) (string, string) {
	d := dir
	for {
		...
		if FileExists(gomod) {
			...
			for _, line := range strings.Split(string(data), "\n") {
				...
				if strings.HasPrefix(line, "module ") {
					return strings.TrimSpace(line[7:]), d
				}
			}
		}
