...
func (s *Scanner) scanAt() (token.Token, error) {
	...
	default:
		return token.ErrorToken, s.assertExpectedString(
			...
			token.AT_CRON.String(),
			token.AT_CRON_RETRY.String())
	}
}
	...
	for {
		...
		case '*':
			...
			case documentOpen, documentHalfClose:
				documentMode = documentHalfClose
			}
		case 0:
			switch documentMode {
			case initMode, documentHalfOpen:
...
func isExtensionFile(filename string) bool {
	return strings.HasSuffix(filename, ".cron") || strings.HasSuffix(filename, ".rabbitmq")
}
