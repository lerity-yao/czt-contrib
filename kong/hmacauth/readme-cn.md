...
基于 [go-zero](https://github.com/zeromicro/go-zero) httpc 封装的 Kong HMAC Auth Go 客户端，自动完成 **HMAC 签名**，内置**熔断器**。遵循 [Kong HMAC Auth 插件](https://developer.konghq.com/plugins/hmac-auth/) 官方规范（基于 [draft-cavage-http-signatures](https://tools.ietf.org/html/draft-cavage-http-signatures) 草案，使用 `@request-target` 伪头）。
...
| 常量 | 值 | 说明 |
|------|------|------|
| `AlgorithmHmacSHA1` | `"hmac-sha1"` | SHA-1，Kong 默认禁用，不推荐使用 |
| `AlgorithmHmacSHA224` | `"hmac-sha224"` | 需要 Kong 3.14+ |
| `AlgorithmHmacSHA256` | `"hmac-sha256"` | **推荐默认** |
...
1. **设置默认头**（未设置时）：`Date`（GMT 格式）、`User-Agent`（默认 `Go-Kong-HmacAuth-Client`）、`Host`（当 host 在签名列表中时）
...
当 `Headers` 包含 `digest` 时，SDK 自动计算并注入 `Digest` 头：

- **算法**：固定使用 SHA-256，格式 `Digest: SHA-256=<base64>`
- **空 body**：同样计算摘要（`sha256.Sum256(nil)`），确保请求完整性可验证
- **form / multipart 跳过**：`application/x-www-form-urlencoded` 和 `multipart/form-data` 类型的 body 不计算 Digest，因为 form 数据在签名前可能被 HTTP transport 重编码

> 当 Kong 端同时启用 `validate_request_body` 时，SDK 的 Digest 头可保证 body 端到端完整性校验。

### 算法选择

| 算法 | 安全性 | 性能 | 适用场景 |
|------|--------|------|----------|
| `hmac-sha1` | 弱（已不推荐） | 快 | 仅兼容旧系统，Kong 默认禁用 |
| `hmac-sha224` | 中 | 较快 | 需要 Kong 3.14+ |
| `hmac-sha256` | 强 | 快 | **推荐默认**，安全性与性能的最佳平衡 |
| `hmac-sha384` | 很强 | 较慢 | 高安全需求场景 |
| `hmac-sha512` | 很强 | 最慢 | 最高安全需求场景 |

> 算法通过 `Conf.Algorithm` 或常量（如 `hmacauth.AlgorithmHmacSHA256`）指定，不传默认 `hmac-sha256`。传入不区分大小写，内部自动转小写。

### 签名头列表配置

`Headers` 决定哪些 header 参与签名。默认只签 `date` 和 `@request-target`，Kong 官方推荐的强签名配置为 `@request-target` + `host` + `date` + `digest`：
...
> `Headers` 中的 header 名会自动转为小写。自定义 header 需要确保请求中已设置对应值，否则签名字符串中该 header 值为空。
